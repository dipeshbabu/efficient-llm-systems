...
- Renamed R-NIAH's synthetic retrieval marker from `password_keyword` to
  ...
  output. Report schema `kv_fidelity.report.v0.3.3` records the renamed field.
...
- `kv-fidelity repeatability` now shares `score` defaults: bundled prompts,
...
- Reports retain schema `kv_fidelity.report.v0.3.2`; only affected Axis A and
...
- SGLang is exposed by every CLI backend selector, and completion/trajectory
  now share tokenizer-generated chat-template IDs.
- Low-confidence R-NIAH results remain visible but are excluded from the
...
- Packaging, Apache-2.0 metadata, source dependencies, CI coverage, and
  documentation now agree on the repository's KV Fidelity/TurboQuant boundary.
...
- `kv-fidelity score --prompts ...` is no longer required. Defaults to the
  `src/kv_fidelity/prompts/v0.1.jsonl` shipped inside the wheel (resolved via
...
- **`KV_FIDELITY_LLAMA_EXTRA_FLAGS` env var** — appended to every
  `llama-cli` / `llama-completion` / `llama-perplexity` subprocess.
  Lets users on constrained VRAM run KV Fidelity against large MoE
  models without forking the runner (e.g.
  `KV_FIDELITY_LLAMA_EXTRA_FLAGS="-ngl 28 -ncmoe 32"` on a 12 GB 3060
  running Qwen3.6-35B-A3B). llama.cpp's last-wins rule means the
  user's `-ngl` overrides KV Fidelity's default `-ngl 99`. Parsed with
...
- **vLLM backend rewritten from skeleton to working implementation**
  (`kv_fidelity/backends/vllm.py`). Cached in-process LLM with
  ...
  `prompt_logprobs`. Env knobs: `KV_FIDELITY_VLLM_GPU_MEMORY_UTILIZATION`,
  `KV_FIDELITY_VLLM_MAX_NUM_SEQS`, `KV_FIDELITY_VLLM_KLD_TOPK`,
  `KV_FIDELITY_VLLM_MAX_MODEL_LEN`.

- **SGLang backend new** (`kv_fidelity/backends/sglang.py`). HTTP-based —
  the user runs an SGLang server separately (typically via the
  published Docker image) and KV Fidelity posts to it. Single-server mode
  (`KV_FIDELITY_SGLANG_URL`) for completion / trajectory / tokenize, plus
  dual-server mode (`KV_FIDELITY_SGLANG_REF_URL` + `KV_FIDELITY_SGLANG_CAND_URL`)
...
- **New extras**: `pip install -e .[vllm]` (vllm),
  `pip install -e .[sglang]` (requests + transformers).

### Verified end-to-end

Cross-engine bench on AMD MI300X / ROCm 7.2 (Qwen3.6-35B-A3B hybrid GDN
+ attention MoE), all four KV Fidelity axes (Trajectory + KLD + R-NIAH +
PLAD) on llama.cpp, vLLM, and SGLang. See
[`research/papers/cross-engine-mi300x.md`](../../research/papers/cross-engine-mi300x.md)
...
- **`kv-fidelity score --html-out path.html`** — single self-contained HTML
  ...
  Implementation: `kv_fidelity/report_html.py`, ~400 LOC. Module is
  importable independently if a script wants HTML without the rest of
  the CLI.

- **`kv-fidelity repeatability` subcommand** — runs the same scoring
...
End-to-end push to make KV Fidelity runnable by someone other than the
author. Concrete deliverables:

- **Backend abstraction** at `kv_fidelity/backends/`. `Backend` ABC defines
...
- **`--backend {auto,llamacpp,mlx,vllm}` flag** routed end-to-end:
  ...
  mlx). Override via `KV_FIDELITY_BACKEND` env.

- **`kv-fidelity selftest` subcommand.** 30-second preflight checking
...
- **`kv-fidelity compare` subcommand.** Multi-report side-by-side table.
...
- **Framework version + environment metadata in JSON** (schema bumped
  to `kv_fidelity.report.v0.3.1`). New top-level fields:
...
on both K and V destroy per-head distribution geometry). KV Fidelity
...
schema bumped to `kv_fidelity.report.v0.2.0`.
...
| `report.py` | `text_report()` and `json_report()` accept optional `rniah` and `plad` results. Per-axis lines for axes C and D added when present. Per-cell R-NIAH breakdown and per-perturbation PLAD breakdown rendered as separate diagnostics blocks. JSON schema bumped to `kv_fidelity.report.v0.2.0`. | Layman + techie clarity at 4-axis scale. |
...
```
 KV Fidelity score    :  78.66  [###############################---------]  DEGRADED
...
| File | Change | Why |
|---|---|---|
| `tools/completion/completion.cpp` (llama.cpp fork) | Surgical patch: when `KV_FIDELITY_TRAJECTORY=<path>` env var is set, write one JSONL record per sampled token (`{"step":N,"token_id":ID}`) right after `common_sampler_sample`. ~12 lines, no new CLI flag. | Decode-time token ID capture, no detokenize round-trip. Off by default; trip-wire on env var so existing users see no behavior change. |
...
| `runner.py` | New `run_completion_trajectory()` wrapper that opens a tmpfile, sets `KV_FIDELITY_TRAJECTORY`, runs `llama-completion`, parses the JSONL, deletes the tmpfile. Raises with a clear message if the patched binary isn't present (empty trajectory file). | Subprocess plumbing for the new axis, isolated from existing `run_completion()` (which still drives `llama-cli` for back-compat). |
...
`text_report()` and `json_report()` v0.1.4 additions:
  ...
  - **JSON schema bumped to `kv_fidelity.report.v0.1.4`**. Adds top-level
...
- **No matrix re-run.** The v0.1.4 axis is implemented and smoke-tested
  ...
  axis. That's a Dipesh-runs-it follow-up — clean numbers will replace the
...
| `report.py` | Schema bumped: `kv_fidelity.report.v0.1.1` → `kv_fidelity.report.v0.1.3` | Both the GTMResult shape and the corpus-identity field changed; the schema number is the integration contract. |
...
| `report.py` | Flatten `composite` to a scalar at the JSON top level; full breakdown moves to `composite_detail` | Aggregators expected `d['composite']` to be a number per the spec mockups. v0.1 nested it as a dict. Bumped JSON schema to `kv_fidelity.report.v0.1.1`. |
| `/tmp/kv_fidelity-matrix.sh` | Per-model `timeout 1800` → `timeout 7200` | Mistral-Small-24B's KLD axis needed >30 min. With real generation now (not banner echo) GTM also takes longer per call. 2 hours per model is the new safety margin. |
