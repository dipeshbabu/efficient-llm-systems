...
- @varjoranta followed up with [fix PR #1](https://github.com/vibhavagarwal5/vllm/pull/1) (2026-04-02): changed default `value_quant_bits` from 4 to 8 (FP8 E4M3). One-line fix that resolves 0% gsm8k and garbage output. @MidasMining independently confirmed 100% on 14-check reasoning benchmark with FP8 values. Cites @dipeshbabu (turbo4-resurrection) as source for "value precision is the quality bottleneck." Note: their fix uses more bits; our approach achieves better compression at equal quality via WHT rotation
...
- Independent tester on independent fork (spiritbuun, not dipeshbabu), further reducing single-implementation bias
...
- Follow-up comparing spiritbuun fork against turboquant_plus with @signalnine's V12 fused single-phase mmvq kernel (shared memory instead of global scratch buffer). **signalnine authored the kernel; dipeshbabu collaborated on integration.**
...
- Re-ran full 65-task benchmark at temp=0 (deterministic) following @dipeshbabu's suggestion for proper KV cache fidelity isolation
...
- Confirmed @dipeshbabu's Qwen3.5-35B result: identical F16/turbo3 accuracy with 2.7x compression
