...
class TestKVCacheCompressor:
    ...
    def test_round_trip_shape(self):
        ...
        assert compressed.packed_nbytes < k.nbytes + v.nbytes
    ...
    def test_memory_stats(self):
        ...
        n_vectors = 32 * 32 * 1024
        assert stats["original_mb"] == pytest.approx(
            n_vectors * 128 * 2 * 2 / 1024 / 1024
        )
        assert stats["compression_ratio"] > 4.0
        assert stats["compressed_mb"] < stats["original_mb"]
        assert stats["storage_model"].startswith("bit_packed")
