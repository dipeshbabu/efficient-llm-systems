...
class TestCompressedSizeBits:
    ...
    def test_size_calculation(self):
        ...
        # Two float32 norms are stored: vector norm + QJL residual norm.
...
class TestCompressionRatio:
    ...
    def test_packed_round_trip_matches_unpacked(self):
        tq = TurboQuant(d=128, bit_width=3, seed=42)
        x = np.random.default_rng(5).standard_normal((7, 128))
        unpacked = tq.dequantize(tq.quantize(x))
        packed = tq.quantize_packed(x)
        reconstructed = tq.dequantize_packed(packed)
        np.testing.assert_allclose(reconstructed, unpacked, rtol=1e-6, atol=1e-6)
        assert packed.mse_indices.dtype == np.uint8
        assert packed.qjl_signs.dtype == np.uint8
