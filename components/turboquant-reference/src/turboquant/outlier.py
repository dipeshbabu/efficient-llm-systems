...
Examples:
- 2.5-bit: 50% channels at 3 bits + 50% at 2 bits = 2.5 bits average
...
class OutlierTurboQuant:
    ...
    def __init__(self, d: int, target_bits: float, seed: int = 42):
        ...
        # The first quantization call calibrates a shared channel map from mean
        # absolute activation magnitude. Call ``calibrate`` explicitly with
        # representative layer samples when the calibration set matters.
        self.outlier_idx = np.arange(n_outlier)
        self.normal_idx = np.arange(n_outlier, d)
        self.calibrated = n_outlier == 0 or n_normal == 0
    ...
    def calibrate(self, samples: np.ndarray) -> None:
        """Select high-precision channels from representative activations.

        The map is shared by subsequently compressed vectors, so it adds no
